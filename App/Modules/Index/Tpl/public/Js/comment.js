...
$(function () {
	...
	$('#send-btn').click(function()
	{
		...
		$.post(handleurl,{
			...
			if(data.status)
			{
				var str='<dl class="paper a';
				str+=Math.abs(Math.random().toFixed(1)*10-5)+1;
				str+='" style="left: 5px; top: 5px; opacity: 1;">';
				str +='<dt><span class="username">'+data.username+'</span>';
				str +='<span class="num">No.'+data.id+'</span></dt>';
				str +='<dd class="content">'+data.content+'</dd>';
				str +='<dd class="bottom"><span class="time">'+data.time+'</span><a href="" class="close"></a></dd></dl>';
				$('#main').append(str);
				$('#close').click();
